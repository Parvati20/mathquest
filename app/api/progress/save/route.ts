...
export async function POST(request: Request) {
  ...
  if (type === "mock") {
    const { correctCount, score } = body as Record<string, unknown>;

    if (typeof correctCount !== "number" || typeof score !== "number") {
      return NextResponse.json({ error: "Invalid fields for mock" }, { status: 400 });
    }

    await saveMockTest(session.user.email, correctCount, score);
    return NextResponse.json({ ok: true });
  }
