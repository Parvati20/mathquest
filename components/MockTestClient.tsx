...
export default function MockTestClient() {
  ...
  const weakTopics = Object.entries(
    ...
    .map(([topic]) => ({
      id: topic,
      title: getLocalizedTopicContent(topic as keyof typeof topicsData, language).title,
    }));
    ...
    return (
      <main className="relative min-h-screen overflow-hidden bg-[#F8FAFC] font-sans text-gray-900">
        ...
        <nav className="relative flex items-center justify-between border-b border-white/70 bg-white/70 px-8 py-4 backdrop-blur-md">
          ...
          <Link href="/tool" className="text-sm font-semibold text-[#E91E63] hover:text-[#c2185b]">
            {text.backToTopics}
          </Link>
        </nav>
          ...
          <div className="relative rounded-[2rem] border border-white/70 bg-white/70 p-8 shadow-[0_24px_60px_rgba(15,23,42,0.08)] backdrop-blur-xl">
            ...
            <div className="mx-auto mt-4 grid max-w-3xl grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-2xl border border-emerald-200/70 bg-emerald-50/90 p-4 shadow-[0_0_20px_rgba(16,185,129,0.16)]">
                <p className="text-sm font-bold text-emerald-700">{text.correct}</p>
              ...
              <div className="rounded-2xl border border-rose-200/80 bg-rose-50/90 p-4 shadow-[0_0_20px_rgba(244,63,94,0.16)]">
                ...
                <p className="mt-1 text-3xl font-black text-rose-700">{wrongCount}</p>
              </div>
            </div>
            ...
            <div className="mx-auto mt-6 max-w-3xl rounded-2xl border border-white/80 bg-white/80 p-4 text-left shadow-[0_8px_28px_rgba(15,23,42,0.06)]">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {weakTopics.length > 0 ? weakTopics.map((topic) => (
                  <div key={topic.id} className="flex items-center gap-2 rounded-full border border-[#E91E63]/20 bg-[#fff1f6] px-3 py-1 text-xs font-bold text-[#E91E63]">
            ...
            {wrongAttempts.length > 0 ? (
              <div className="mx-auto mt-6 max-w-3xl space-y-3 text-left">
                {wrongAttempts.map((item) => (
                  <div key={item.question.id} className="rounded-2xl border border-white/80 bg-white p-4 shadow-[0_8px_26px_rgba(15,23,42,0.05)]">
                    <div className="mb-2 flex items-center justify-between gap-3">
                      ...
                      <span className="text-[11px] font-semibold text-slate-400">{getLocalizedTopicContent(item.question.topic as keyof typeof topicsData, language).title}</span>
                    </div>
                    <p className="text-sm font-bold text-slate-700">{getLocalizedQuestion(item.question, language).question}</p>
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-[#F8FAFC] font-sans text-gray-900">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute left-[-10%] top-[-8%] h-[24rem] w-[24rem] rounded-full bg-[radial-gradient(circle,_rgba(233,30,99,0.10)_0%,_rgba(233,30,99,0.02)_42%,_transparent_72%)] blur-3xl" />
        <div className="absolute right-[-8%] top-[10%] h-[22rem] w-[22rem] rounded-full bg-[radial-gradient(circle,_rgba(59,130,246,0.08)_0%,_rgba(59,130,246,0.02)_45%,_transparent_74%)] blur-3xl" />
        <span className="animate-float absolute left-[15%] top-[22%] text-5xl opacity-[0.06]">⏱️</span>
        <span className="animate-float absolute right-[18%] top-[32%] text-5xl opacity-[0.05]" style={{ animationDelay: "1.5s" }}>✅</span>
        <span className="animate-float absolute left-[28%] bottom-[22%] text-5xl opacity-[0.05]" style={{ animationDelay: "2.2s" }}>⏱️</span>
      </div>
      ...
      <section className="relative mx-auto max-w-3xl px-6 py-10">
        <div className="flex items-center justify-between text-sm text-gray-600">
          ...
          <span className="rounded-full border border-[#E91E63]/15 bg-white/80 px-3 py-1 text-xs font-bold text-[#E91E63] shadow-[0_10px_20px_rgba(233,30,99,0.08)]">15:00 {text.timerRunning}</span>
        </div>
          ...
          <div className="mt-6 space-y-3">
            {localizedCurrentQuestion?.options.map((option, optionIndex) => {
              ...
              return (
                ...
                >
                  <span className={`inline-flex h-9 w-9 items-center justify-center rounded-full text-sm font-black text-white shadow-[0_12px_24px_rgba(15,23,42,0.10)] ${
                    picked
