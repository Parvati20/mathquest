  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-4 left-4 z-[70] grid h-12 w-12 place-items-center rounded-2xl border border-[#E91E63]/25 bg-white text-[#E91E63] shadow-[0_12px_30px_rgba(233,30,99,0.24)] backdrop-blur-md lg:hidden"
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-5 w-5">
          {mobileMenuOpen ? (
            <>
      ...
      {mobileMenuOpen ? (
        <div className="fixed inset-0 z-[65] lg:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-[280px] max-w-[86vw] overflow-y-auto border-r border-gray-100 bg-white p-4 shadow-[20px_0_50px_rgba(15,23,42,0.2)]">
            <div className="flex items-center justify-between">
              <Link href="/tool" className="flex items-center" onClick={() => setMobileMenuOpen(false)}>
                <BrandLogo />
              </Link>
              <button
                type="button"
                aria-label="Close menu"
                onClick={() => setMobileMenuOpen(false)}
                className="rounded-lg border border-gray-100 bg-white p-1.5 text-slate-500"
              >
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4">
                  <line x1="18" y1="6" x2="6" y2="18" />
                  <line x1="6" y1="6" x2="18" y2="18" />
            ...
            <div className="mt-4 rounded-xl border border-white/60 bg-white/90 px-3 py-2.5 shadow-[0_8px_24px_rgba(15,23,42,0.04)]">
              <p className="text-[9px] font-semibold uppercase tracking-wider text-slate-400">Total Points</p>
              <p className="mt-0.5 text-sm font-black text-slate-900">{totalPoints}</p>
            </div>
              ...
              {navItems.map((item) => (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold transition-all ${
                    item.active
                      ? "border border-[#E91E63]/20 bg-[#fff1f6] text-[#E91E63]"
            ...
            <div className="mt-4 flex items-center gap-2.5 rounded-xl border border-gray-100 bg-white p-2.5">
              {session.user?.image ? (
                <Image
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-xs font-bold leading-none text-slate-900">{profileName}</p>
                <p className="mt-0.5 truncate text-[10px] leading-none text-slate-400">{session.user?.email}</p>
              </div>
              <button
