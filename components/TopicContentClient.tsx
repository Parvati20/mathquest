...
function createQuizFromExample(question: string, answer: string) {
  ...
  const shuffled = [raw[0], raw[1], raw[2], raw[3]].sort(() => Math.random() - 0.5);
  const correctIndex = shuffled.indexOf(correctValue);
  const options = shuffled.map((v) => String(v));
...
function getWorkedSteps(topic: string, question: string, answer: string) {
  ...
  if (topic === "percentage" && numbers.length >= 2) {
    ...
    const percentFromQuestion = percentMatch ? Number(percentMatch[1]) : null;

    const percent =
      typeof percentFromQuestion === "number" && Number.isFinite(percentFromQuestion)
        ? percentFromQuestion
        : numbers[0];

    const whole =
      typeof percentFromQuestion === "number" && Number.isFinite(percentFromQuestion)
        ? (numbers.find((value) => value !== percentFromQuestion) ?? numbers[1])
        : numbers[1];
          ...
          <div className="mb-8 rounded-[2rem] border border-white/50 bg-white/45 px-6 py-8 shadow-[0_30px_80px_rgba(148,163,184,0.14)] backdrop-blur-2xl">
            ...
            <div className="grid gap-5 lg:grid-cols-[1.05fr_0.95fr]">
              <section className="rounded-[1.75rem] border border-white/50 bg-white/60 p-6 shadow-2xl shadow-slate-200/35 backdrop-blur-xl">
                <p className="mb-4 flex items-center gap-2 text-xs font-black uppercase tracking-[0.22em] text-[#E91E63]">
                  💡 Secret Hack
                </p>
                <h2 className="mb-5 text-xl font-black text-gray-900">{text.theory}</h2>
                  ...
                  {data.theory
                    ...
                    .map((paragraph, idx) => (
                      <div key={idx} className="group">
                        ...
                        <div className="relative rounded-xl border border-transparent bg-gradient-to-r from-[#E91E63]/5 to-[#FF8A65]/5 p-4 transition-all hover:border-[#E91E63]/30 hover:from-[#E91E63]/10 hover:to-[#FF8A65]/10 hover:shadow-[0_0_18px_rgba(233,30,99,0.15)]">
                          <div className="absolute left-0 top-0 bottom-0 w-1 rounded-l-lg bg-gradient-to-b from-[#E91E63] to-[#FF8A65] opacity-0 transition-opacity group-hover:opacity-100" />
                          
                          {/* Icon indicator */}
                          <div className="mb-2 flex items-start gap-3">
                            <span className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-[#E91E63]/15 text-sm font-black text-[#E91E63]">
                              {idx === 0 ? "🎯" : idx === 1 ? "🔑" : idx === 2 ? "💪" : "⚡"}
                            </span>
